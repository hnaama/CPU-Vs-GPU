...
void PixelBuffer::drawRectangle(int x, int y, int w, int h, uint32_t color) {
    // Draw four edges
...
void PixelBuffer::drawCircle(int cx, int cy, int radius, uint32_t color) {
...
void PixelBuffer::fillCircle(int cx, int cy, int radius, uint32_t color) {
...
std::pair<int, int> PixelBuffer::project3DTo2D(const Vec3& point, int screenWidth, int screenHeight) {
    ...
    int x = (int)((point.x + 1.0f) * screenWidth * 0.5f);
    int y = (int)((1.0f - point.y) * screenHeight * 0.5f);
    return {x, y};
}